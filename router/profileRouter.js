...
router.put("/follow/:profileId", authenticate, async (request, response) => {
  try {

    let secondProfileId = request.params.profileId;
    var userId = request.user.id;
    let followerProfile = await Profile.findById(secondProfileId);
    var secondUserId= followerProfile.user._id;
    
    let followUser = await User.findById(userId);
    let followingProfile = await Profile.findOne({ user: followUser });
    var isfollowed = followingProfile.following.includes(secondUserId);

    var option = isfollowed ? "$pull" : "$addToSet";

    // Insert/delete post like
    let profile1 = await Profile.findOneAndUpdate(
      { user: followUser },
      { [option]: { following: secondUserId } },
      { new: true }
    );

    let profile2 = await Profile.findByIdAndUpdate(
      secondProfileId,
      { [option]: { followers: userId } },
      { new: true }
    );
   response.status(200).json({ profile: [profile1, profile2] });
  }
